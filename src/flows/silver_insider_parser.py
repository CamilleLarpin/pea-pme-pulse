...
def task_parse_signals_with_ai(text: str, doc_info: dict, api_key: str):
    ...
    logger = get_run_logger()
    logger.info("Initiating AI parsing with Groq LLM...")
...
def task_validate_signals(raw_signals: list, doc: dict):
    ...
    """
    logger = get_run_logger()
    logger.info("Validating and enriching AI-extracted signals...")
    validated = []
    ...
    for sig in raw_signals:
        ...
        if len(name_parts) < 2 or len(name_parts) > 4:
            continue
        if any(w in forbidden_words for w in name_parts):
            continue
...
def task_upload_bq(rows: list, project_id: str, dataset_id: str):
    ...
    logger.info(
        f"Successfully uploaded {len(rows)} signals to {dataset_id}.{GCP_PREFIX_OUTPUT_TABLE_ID}"
    )
...
def amf_insider_parser_flow():
    ...
    pending_docs = task_load_from_bigquery(
        config["project_id"], config["db_bronze"], GCP_PREFIX_INPUT_TABLE_ID
    )
    current_docs = task_load_from_bigquery(
        config["project_id"], config["db_silver"], GCP_PREFIX_OUTPUT_TABLE_ID
    )

...
#   rsi_signal        : RSI_14 < 30 → 2 | 30–70 → 1 | ≥ 70 → 0 | NaN → 1
#   macd_signal       : MACD > MACD_signal → 2 | ≤ → 0 | NaN → 1
#   golden_cross      : SMA_50 > SMA_200 → 2 | ≤ → 0 | NaN → 1
#   bollinger_signal  : Close < BB_lower → 2 | in bands → 1 | > BB_upper → 0 | NaN → 1
...
def compute_stocks_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[df["RSI_14"] < 30, "rsi_signal"] = 2.0
    df.loc[(df["RSI_14"] >= 30) & (df["RSI_14"] < 70), "rsi_signal"] = 1.0
    df.loc[df["RSI_14"] >= 70, "rsi_signal"] = 0.0
    ...
    df.loc[bb_valid & (df["Close"] < df["BB_lower"]), "bollinger_signal"] = 2.0
    df.loc[bb_valid & (df["Close"] > df["BB_upper"]), "bollinger_signal"] = 0.0

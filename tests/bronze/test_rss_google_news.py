...
def test_clean_title_strips_source_suffix():
    assert (
        clean_title("THERMADOR annonce ses résultats - lefigaro.fr")
        == "THERMADOR annonce ses résultats"
    )
    assert (
        clean_title("Hexaom prévoit une hausse - Ouest-France")
        == "Hexaom prévoit une hausse - Ouest-France"
    )  # no domain, unchanged
    assert (
        clean_title("NFL Biosciences s'introduit en bourse - euronext.com")

...
from bronze.rss_abcbourse import import_rss, rss_sources
    ...
    if total_matches > 0:
        for category, entries in filtered_results.items():
            for entry in entries:
                ...
                logger.success(
                    f"[{category}] Match: {company} (ISIN: {isin}) "
                    f"Score: {score}% -> {title[:75]}..."
                )
    else:
        logger.info(
            "No matches found. This is normal if today's news doesn't mention the mock companies."
        )

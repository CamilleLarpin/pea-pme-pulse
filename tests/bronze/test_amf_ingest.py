...
import src.bronze.amf_ingest as mod
...
def test_build_where_clause_ok() -> None:
    ...
    assert result == (
        "identificationsociete_iso_cd_isi IN "
        "('FR0000120271','US0378331005')"
    )
...
def test_inject_bq_calls_expected_steps(mocker, sample_config: mod.Config) -> None:
    ...
    truncate_staging = mocker.patch.object(mod, "truncate_staging_table")
    ...
    truncate_staging.assert_called_once_with(fake_client, sample_config.full_staging_table_id)
